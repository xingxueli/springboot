...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    public Menu addMenu(Menu menu) {
        ...
        if(menu.getParentId() == null || menu.getParentId().longValue() == ConstantUtil.rootParentId){
            menu.setLevel(1);//根节点层级为1
            menu.setPath(null);//根节点路径为空
            menu.setParentId(ConstantUtil.rootParentId);
        }else{
            ...
            if(StringUtils.isNotEmpty(parentMenu.getPath())){
                menu.setCatalog(parentMenu.getCatalog() + "," + menu.getName());
            }else{
                menu.setCatalog(parentMenu.getName());
            }
        }
        //可以使用雪花算法，生成ID
//        menu.setId(System.currentTimeMillis());
    ...
    public List<MenuVo> queryMenuTree() {
        ...
        List<MenuVo> resultList = transferMenuVo(allMenu, ConstantUtil.rootParentId);
        return resultList;
    }
        ...
        if(!CollectionUtils.isEmpty(userRoles)){
            ...
            if(!CollectionUtils.isEmpty(roleMenus)){
                ...
                if(!CollectionUtils.isEmpty(menus)){
                    ...
                    List<MenuVo> resultList = transferMenuVo(allMenus, ConstantUtil.rootParentId);
                    return resultList;
                }
            }
        ...
        if(!CollectionUtils.isEmpty(allMenu)){
            for (Menu source : allMenu) {
                if(parentId.longValue() == source.getParentId().longValue()){
                    ...
                    menuVo.setCatalogList(Arrays.asList(StringUtils.split(source.getCatalog(),",")));
                    //查询子菜单，并封装
